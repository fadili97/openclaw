...
**Deploy from a GitHub repo or fork:** In Railway, create a new project → **Deploy from GitHub repo** → select `openclaw/openclaw` or your fork (e.g. `fadili97/openclaw`). The repo includes a `Dockerfile` and `railway.toml`; use the same checklist below (Volume, Variables, HTTP Proxy on port 8080).
